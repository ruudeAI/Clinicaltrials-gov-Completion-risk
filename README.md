...
A machine learning project that uses [ClinicalTrials.gov](https://clinicaltrials.gov/) API v2 data to estimate whether a cancer-related clinical trial is likely to **complete** or be **at risk** of termination, withdrawal, or suspension.
...
| **Scope** | Cancer-related clinical trials (MVP) |
...
| `intervention_types` | Drug, biological, device, procedure, etc. |
...
│
├── data/
│   ├── raw/                           ← Raw API data (git-ignored)
│   ├── processed/                     ← Cleaned dataset (git-ignored)
...
After training, the model generates a detailed evaluation report at `reports/results_summary.md` including:
...
- **Cancer trials only** — the MVP is trained on cancer trials and may not generalize to other areas
